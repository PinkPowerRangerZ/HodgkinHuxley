...
def compare_running_times(I_values, T):
    ...
    return execution_time_parallel, execution_time_sequential


if __name__ == '__main__':
    # Define the range of applied current values and simulation duration
    I_values_range = [(0, 20),(80,100),(140,160),(200,220)]  # Different ranges of applied current values
    ...
    for I_range in I_values_range:
        ...
        parallel_time, sequential_time = compare_running_times(I_values, T)

        table_data.append([I_range[0], I_range[1], parallel_time, sequential_time])

    table = pd.DataFrame(table_data, columns=['I_values_start', 'I_values_end', 'Parallel Time', 'Sequential Time'])

    print(table)
    table.to_csv("intervals.csv")
